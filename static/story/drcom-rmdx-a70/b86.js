	window.onload=function(){
		...
		if(cook!=null)
		{
			...
			$.post('../unonline_user.php', 'action=auto_login&filename='+cook, function(data){
				if(data == 0){
					//$("#errormsg").html("保存的信息已失效").show();
				} else {
					var arr = data.split("|");
					if(document.form1){
						document.form1.uname.value=arr[0];
						if(document.getElementById( "show_save_me").style.display == "block")
						{
     ...
     function jump_to(url1)
     {
      ...
      function do_login()
      {
		...
		var return_http_host = document.form1.return_http_host.value;
		var server_type = document.form1.server_type.value;
		var reg_uname=/^[A-Za-z0-9]*$/; 
		//var reg_uname=/[^\w\.\/]/;
		...
		if(return_http_host == "http://go.ruc.edu.cn/index.php"){
    		//登录微人大
    		var frame = document.getElementById("vruciframe");
		 	frame.src = 'https://ruc.tiup.cn/Login/login?email='+uname+'&pass='+pass;
		...
		var data="username="+uname+"&password={TEXT}"+encodeURIComponent(pass)+"&drop="+drop+"&type=1&n=100";
      	...
      	if(p.test(con))
      	{
      		...
      		{
				var cookie_info = hex_md5(uname);
      			setCookie("srun_login",cookie_info);
				//保存信息
				$.post('../unonline_user.php', 'action=save_user_info&username='+uname+'&password='+pass+'&filename='+cookie_info, function(data){
					if(data == 1){
						 window.location.href = return_http_host;
					}
				});
      		}
      		else
			...
			if(getCookie("srun_login") == null){//cookie已删除或者已过期的情况下，删除对应的文件
				$.post('../unonline_user.php', 'action=delete_auto_file&filename='+hex_md5(uname), function(data){});
			}
			// if(return_http_host == "http://go.ruc.edu.cn/index.php"){
			// submitInfo();
			// }
			...
			if (con == "status_error" || con == "online_num_error" || con == "flux_error" || con == "minutes_error") {
				var server_ip = '';
				if(server_type == 0){
					server_ip = "http://go.ruc.edu.cn";
				}else if(server_type == 1){
					server_ip = "http://go.ruc.edu.cn/phone";
				}
				if(server_ip != ''){
					$.ajax({
						 type: "get",
						 url: server_ip+"/change_user_balance.php?action=get_user_data&username="+uname+"&password="+pass,
						 dataType: "jsonp",
						 success: function (val) {
							 if(val == 100)
							 {
								 window.location.href = return_http_host+'?error='+con;
							 }
						 }
					 });
				}
...
// 	frame.src = 'https://ruc.tiup.cn/Login/login?email='+txt+'&pass='+pass;
// 	if(frame.attachEvent){
// 		frame.attachEvent("onload", function(){
// 			document.getElementById("form1").submit();
// 			//document.forms["login_form"].submit();
